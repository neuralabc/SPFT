...
% structure,
%
% Modified to account for missing maxVal settings in erroneous log files
% (this is custom for each participant that misses the Maxwert_<NUM> data)
%
% EACH presentation of the stimulus takes 12.5ms, for a total duration
...
%
% for file in *Daily*log ; do cmd="sed -i.bak 's/NA//' $file" ; echo $cmd ; done
%
% ***===***===***===***===***===***===***===***===***===***===***===***===
% ========================================================================
...
% does not APPEAR to affect the results, but will of course affect
% computation of actual force values.

% P10 has very high lag on 1st trial of d2, looks like they did not realise
% that the trial started so are scored rather poorly, recommend dropping
% this trial
%####################################################################################################################################################################
%IDs={'P34'};
%subjectDirs = IDs;
...
%        temp.fname=strcat(SPFT.dataDir,subjectDirs{ID},'/',IDs{ID},SPFT.fileNameTail); %figure out what the trial name should be
        ...
        if exist(temp.fname,'file')==2
            ...
            %XXX TODO: this will cut out a maxwert that has 99 in it :-(
            ...
            %%%=====================================================================================================%%%
            %%% 2015, 09 Christopher Steele
            %%% change to code
            %%% encase identification of Maxwert text in try/catch statements to account for rare cases (P10, P11) where the program did not collect them  
            %%%=====================================================================================================%%%
            ...
            try
                ...
                warning('Maxwert_Kg not identified in data file. Currently set to 0')
                temp.maxVal_Kg=0;
            end
            ...
            try
                ...
                warning('Maxwert_8bit not identified in data file. Currently set to 0')
                temp.maxVal_8bit=0;
            end
            ...
            try
                ...
                warning('Maxwert_ not identified in data file.')
                warning('-- THIS SETTING IS CRUCIAL, YOU NEED TO MANUALLY ADD THE CORRECT VALUE IN THE SCRIPT')
                return %we need to add individual checks as below to correct this issue if they arise. The examples below are from the SPFT 7T dataset
                %add the appropriate check and setting of temp.maxVal for
                %the particular day that this setting needs to be added
                %manually - unknown cause of this error.
%                 if strcmp(IDs{ID},'P10') && day==4
%                     temp.maxVal=322; %Sophia: corrected this value, think it` s 322, not 305 (that` s the value for P11)
%                     fprintf('%%========================================================%%\n');
%                     fprintf('%% temp.maxVal manually set to %i for %s, day %i\n',temp.maxVal,IDs{ID},day);
%                     fprintf('%%========================================================%%\n\n');
%                 elseif strcmp(IDs{ID},'P11') && day==4 %for P11 maxvalue is also missing
%                     temp.maxVal=305;
%                     fprintf('%%========================================================%%\n');
%                     fprintf('%% temp.maxVal manually set to %i for %s, day %i\n',temp.maxVal,IDs{ID},day);
%                     fprintf('%%========================================================%%\n\n');
%                 elseif strcmp(IDs{ID},'P17') && day==3 %for P11 maxvalue is also missing
%                     temp.maxVal=444;
%                     fprintf('%%========================================================%%\n');
%                     fprintf('%% temp.maxVal manually set to %i for %s, day %i\n',temp.maxVal,IDs{ID},day);
%                     fprintf('%%========================================================%%\n\n');
%                  elseif strcmp(IDs{ID},'P17') && day==4 %for P11 maxvalue is also mP10issing
%                     temp.maxVal=444;
%                     fprintf('%%========================================================%%\n');
%                     fprintf('%% temp.maxVal manually set to %i for %s, day %i\n',temp.maxVal,IDs{ID},day);
%                     fprintf('%%========================================================%%\n\n');
%                  elseif strcmp(IDs{ID},'P18') && day==4 %for P11 maxvalue is also missing
%                     temp.maxVal=246;
%                     fprintf('%%========================================================%%\n');
%                     fprintf('%% temp.maxVal manually set to %i for %s, day %i\n',temp.maxVal,IDs{ID},day);
%                     fprintf('%%========================================================%%\n\n');
%                 else warning('-- STOPPING EXECUTION so that you can go fix the script file ;-) .')
%                     temp.maxVal=0;
%                     return
%                 end
            end
            ...
            if length(temp.setWeight) == 1 %there should be only ONE setting because we SPECIFIED the range (previously, 2 were recorded because we set to default (15kg) and then reset when Maximalkraft was done)
                if strcmp(temp.setWeight{1},'set_range_938g')
                    temp.sensorMaxWeight=.9375;
                    temp.rangeFactor=1;
                elseif (strcmp(temp.setWeight{1},'set_range_1p875kg') || strcmp(temp.setWeight{1},'set_range_1875g'))
                    temp.sensorMaxWeight=1.875;
                    temp.rangeFactor=2;
                elseif strcmp(temp.setWeight{1},'set_range_3p75kg')
                    temp.sensorMaxWeight=3.75;
                    temp.rangeFactor=4;
                elseif strcmp(temp.setWeight{1},'set_range_7p5kg')
                    temp.sensorMaxWeight=7.5;
                    temp.rangeFactor=8;
                elseif strcmp(temp.setWeight{1},'set_range_15kg')
            ...
            temp.GLM.SMPblocks=[temp.GLM.SMPblocks ones(length(temp.GLM.SMPblocks),1)*SPFT.blockDuration ones(length(temp.GLM.SMPblocks),1)];
            temp.GLM.LRNblocks=[temp.GLM.LRNblocks ones(length(temp.GLM.LRNblocks),1)*SPFT.blockDuration ones(length(temp.GLM.LRNblocks),1)];
            ...
            temp.out.trial.rawForce=cell(SPFT.totalTrials,1);
            ...
            for trial=1:length(temp.trialOn)
                ...
                temp.trialOffTime=temp.trialOnTime+SPFT.seqDur*10000; %
                ...
                %plot(temp.data(temp.trialOn(trial):temp.trialOffIdx,1)); pause;
            ...
            % XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
            temp.fMRI_vol_start=temp.fMRIonset(1:2:end);%XXX DIVIDE BY 2 BECAUSE VASO AND BOLD ARE EACH COLLECTED PER TR
            temp.fMRI_vol_stop =temp.fMRI_vol_start+(SPFT.TR*10000);
            temp.out.rawForce_per_TR=zeros(length(temp.fMRI_vol_start),1)*NaN; %XXX DIVIDE BY 2 BECAUSE VASO AND BOLD ARE EACH COLLECTED PER TR
            ...
            temp.out.SMP=[temp.out.trial.remappedSensorVal(SPFT.trialTypes==1) temp.out.trial.rawTime(SPFT.trialTypes==1) temp.out.trial.remappedForce(SPFT.trialTypes==1) temp.out.trial.rawForce(SPFT.trialTypes==1)]; %FORCE and then Time
            temp.out.RST=[temp.out.trial.remappedSensorVal(SPFT.trialTypes==2) temp.out.trial.rawTime(SPFT.trialTypes==2) temp.out.trial.remappedForce(SPFT.trialTypes==2) temp.out.trial.rawForce(SPFT.trialTypes==2)];
            temp.out.LRN=[temp.out.trial.remappedSensorVal(SPFT.trialTypes==3) temp.out.trial.rawTime(SPFT.trialTypes==3) temp.out.trial.remappedForce(SPFT.trialTypes==3) temp.out.trial.rawForce(SPFT.trialTypes==3)];
            ...
            SPFT.fullData{ID,day}=temp; %create a new data structure to hold the full data for each individual from their log files
            fprintf('----------------------------------------------------\n\n');
        ...
        cmd=['SPFT.' IDs{ID} '.d' num2str(day) '=temp;'];
        eval(cmd)
...
clear temp;
temp.temp=[];
temp.out.all.rmseLRN=cell(length(SPFT.IDs),SPFT.numDays);
...
for ID=1:length(IDs)
    % since this only loops over the index values, IT WILL GIVE YOU
    % INCORRECT RESULTS if you re-run it after changing the list of IDs
    fprintf('--- Processing/Scoring %s ---\n',IDs{ID});
    temp.out.rmseLRN=repmat(NaN,1,SPFT.numBlocks*SPFT.numTrials);
    temp.out.rmseSMP=repmat(NaN,1,SPFT.numBlocks*SPFT.numTrials);
    ...
    temp.out.velaccjrkLRN=repmat(NaN,3,SPFT.numBlocks*SPFT.numTrials);
    temp.out.velaccjrkSMP=repmat(NaN,3,SPFT.numBlocks*SPFT.numTrials);
    ...
    for day=1:SPFT.numDays
        ...
        for trial=1:SPFT.numBlocks*SPFT.numTrials
            ...
            try
                ...
                %of interest
                temp.LRNresp=resample(SPFT.fullData{ID,day}.out.LRN{trial,1},SPFT.seqLength,length(SPFT.fullData{ID,day}.out.LRN{trial,1}));
                temp.SMPresp=resample(SPFT.fullData{ID,day}.out.SMP{trial,1},SPFT.seqLength,length(SPFT.fullData{ID,day}.out.SMP{trial,1}));
                %[temp.temp temp.out.errorlagLRN(trial)]=SPFT_calcTemporalOffset(abs(temp.LRNresp-SPFT.LRN),temp.LRNresp); %xcorr relating the absolute error to the force that is used
                %[temp.temp temp.out.errorlagSMP(trial)]=SPFT_calcTemporalOffset(abs(temp.SMPresp-SPFT.SMP),temp.SMPresp);
                [r p]=corrcoef(abs(temp.LRNresp-SPFT.LRN),temp.LRNresp); %xcorr relating the absolute error to the force that is used
                if p(1,2)>.05
                    r(1,2)=NaN;
                end
                temp.out.errorlagcorrLRN(trial)=r(1,2);
                [r p]=corrcoef(abs(temp.SMPresp-SPFT.SMP),temp.SMPresp);
                if p(1,2)>.05
                    r(1,2)=NaN;
                end
                ...
                temp.out.rmseLRN(trial)=sqrt(sum((temp.LRNresp-SPFT.LRN).^2)./SPFT.seqLength);%/mean(temp.LRNresp);%/range(temp.LRNresp); %different ways of normalising RMSE
                temp.out.rmseSMP(trial)=sqrt(sum((temp.SMPresp-SPFT.SMP).^2)./SPFT.seqLength);%/mean(temp.SMPresp);
                
                [temp.out.normrLRN(trial) temp.out.lagLRN(trial)]=SPFT_calcTemporalOffset(SPFT.LRN,temp.LRNresp);
                [temp.out.normrSMP(trial) temp.out.lagSMP(trial)]=SPFT_calcTemporalOffset(SPFT.SMP,temp.SMPresp);
                
                temp.shiftedLRNdiff=circshift(temp.LRNresp,temp.out.lagLRN(trial))-SPFT.LRN;
                temp.shiftedSMPdiff=circshift(temp.SMPresp,temp.out.lagSMP(trial))-SPFT.SMP;
                ...
                [temp.out.velaccjrkLRN(1,trial) temp.out.velaccjrkLRN(2,trial) temp.out.velaccjrkLRN(3,trial)]=SPFT_calcVelAccJrk(temp.LRNresp,SPFT.resampleFactor);
                [temp.out.velaccjrkSMP(1,trial) temp.out.velaccjrkSMP(2,trial) temp.out.velaccjrkSMP(3,trial)]=SPFT_calcVelAccJrk(temp.SMPresp,SPFT.resampleFactor);
                
                %             [Dist,D,k,w]=dtw(SPFT.LRN(:)',temp.LRNresp(:)'); %dynamic time warping calculation
                %             temp.out.kLRN(trial)=k;
                %             [Dist,D,k,w]=dtw(SPFT.SMP(:)',temp.SMPresp(:)');
        ...
        %temp.out.all.errorlagLRN=[temp.out.all.errorlagLRN; 90/80*temp.out.errorlagLRN/SPFT.seqLength*SPFT.seqDur*1000*-1];%*1/60*-1000];%/SPFT.seqLength*SPFT.seqDur*1000*-1]; %lag (ms) from reference onset
        %temp.out.all.errorlagSMP=[temp.out.all.errorlagSMP; 90/80*temp.out.errorlagSMP/SPFT.seqLength*SPFT.seqDur*1000*-1];%*1/60*-1000];%/SPFT.seqLength*SPFT.seqDur*1000*-1];
        temp.out.all.errorlagcorrLRN{ID,day}=[temp.out.all.errorlagcorrLRN{ID,day}; temp.out.errorlagcorrLRN];%*1/60*-1000];%/SPFT.seqLength*SPFT.seqDur*1000*-1]; %lag (ms) from reference onset
        temp.out.all.errorlagcorrSMP{ID,day}=[temp.out.all.errorlagcorrSMP{ID,day}; temp.out.errorlagcorrSMP];%*1/60*-1000];%/SPFT.seqLength*SPFT.seqDur*1000*-1];
        
        temp.out.all.lagLRN{ID,day}=[temp.out.all.lagLRN{ID,day}; 90/80*temp.out.lagLRN/SPFT.seqLength*SPFT.seqDur*1000*-1];%*1/60*-1000];%/SPFT.seqLength*SPFT.seqDur*1000*-1]; %lag (ms) from reference onset
        temp.out.all.lagSMP{ID,day}=[temp.out.all.lagSMP{ID,day}; 90/80*temp.out.lagSMP/SPFT.seqLength*SPFT.seqDur*1000*-1];%*1/60*-1000];%/SPFT.seqLength*SPFT.seqDur*1000*-1];
...
SPFT.scoredData=temp.out.all;
%clearvars -except SPFT

%save([SPFT.dataDir filesep '2015_04_SPFT_7T_M1_forceCalibration_preProcessedData'],'SPFT','-v7.3')
...
theDataLRN=cell2mat(SPFT.scoredData.alignedrmseLRN(ID,days)); %row=ID, column=trials across all days (i.e., 54)
theDataSMP=cell2mat(SPFT.scoredData.alignedrmseSMP(ID,days));
plotName='RMSE';
yAxisName='Error (scaled absolute deviation)';
SPFT_plotIndividualData(theDataLRN,theDataSMP,plotName,yAxisName,SCALEX);

theDataLRN=cell2mat(SPFT.scoredData.lagLRN(ID,days)); %row=ID, column=trials across all days (i.e., 54)
theDataSMP=cell2mat(SPFT.scoredData.lagSMP(ID,days));
...
IDs=SPFT.IDs;
f=figure('name','RMSE');
theDataLRN2=cell2mat(SPFT.scoredData.rmseLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP2=cell2mat(SPFT.scoredData.rmseSMP); 
errorbar(nanmean(theDataLRN2),nanstd(theDataLRN2)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP2),nanstd(theDataSMP2)./sqrt(length(IDs)),'go-','linewidth',1);
...
legend('Learning Task (LERNEN)','Simple Task (UEBEN)');
ylim([15 50])

figure('name','Lag-aligned RMSE');
theDataLRN3=cell2mat(SPFT.scoredData.alignedrmseLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP3=cell2mat(SPFT.scoredData.alignedrmseSMP); 
errorbar(nanmean(theDataLRN3),nanstd(theDataLRN3)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP3),nanstd(theDataSMP3)./sqrt(length(IDs)),'go-','linewidth',1);

plot([9.5 9.5], [min([nanmean(theDataLRN3), nanmean(theDataSMP3)]),max([nanmean(theDataLRN3), nanmean(theDataSMP3)])],'k:');
plot([18.5 18.5], [min([nanmean(theDataLRN3), nanmean(theDataSMP3)]),max([nanmean(theDataLRN3), nanmean(theDataSMP3)])],'k:');
plot([27.5 27.5], [min([nanmean(theDataLRN3), nanmean(theDataSMP3)]),max([nanmean(theDataLRN3), nanmean(theDataSMP3)])],'k:');
plot([36.5 36.5], [min([nanmean(theDataLRN3), nanmean(theDataSMP3)]),max([nanmean(theDataLRN3), nanmean(theDataSMP3)])],'k:');
plot([45.5 45.5], [min([nanmean(theDataLRN3), nanmean(theDataSMP3)]),max([nanmean(theDataLRN3), nanmean(theDataSMP3)])],'k:');
plot([54.5 54.5], [min([nanmean(theDataLRN3), nanmean(theDataSMP3)]),max([nanmean(theDataLRN3), nanmean(theDataSMP3)])],'k:');
xlabel('Trial');
ylabel('Error (scaled absolute deviation)');
legend('Learning Task (LERNEN)','Simple Task (UEBEN)');

saveas(gcf,'RMSE_LagAligned.png')


figure('name','Force Response');
theDataLRN=cell2mat(SPFT.scoredData.respLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP=cell2mat(SPFT.scoredData.respSMP); 
errorbar(nanmean(theDataLRN),nanstd(theDataLRN)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP),nanstd(theDataSMP)./sqrt(length(IDs)),'go-','linewidth',1);
...
theDataLRN4=cell2mat(SPFT.scoredData.lagLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP4=cell2mat(SPFT.scoredData.lagSMP); 
errorbar(nanmean(theDataLRN4),nanstd(theDataLRN4)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP4),nanstd(theDataSMP4)./sqrt(length(IDs)),'go-','linewidth',1);

plot([15 15], [min([nanmean(theDataLRN4), nanmean(theDataSMP4)]),max([nanmean(theDataLRN4), nanmean(theDataSMP2)])],'k:');  %line separating days : needs to be adjusted in height


%plot([9.5 9.5], [min([nanmean(theDataLRN4), nanmean(theDataSMP4)]),max([nanmean(theDataLRN4), nanmean(theDataSMP2)])],'k:');
%plot([18.5 18.5], [min([nanmean(theDataLRN4), nanmean(theDataSMP4)]),max([nanmean(theDataLRN4), nanmean(theDataSMP2)])],'k:');
%plot([27.5 27.5], [min([nanmean(theDataLRN4), nanmean(theDataSMP4)]),max([nanmean(theDataLRN2), nanmean(theDataSMP2)])],'k:');
%plot([36.5 36.5], [min([nanmean(theDataLRN4), nanmean(theDataSMP4)]),max([nanmean(theDataLRN2), nanmean(theDataSMP2)])],'k:');
%plot([45.5 45.5], [min([nanmean(theDataLRN4), nanmean(theDataSMP4)]),max([nanmean(theDataLRN2), nanmean(theDataSMP2)])],'k:');
%plot([54.5 54.5], [min([nanmean(theDataLRN4), nanmean(theDataSMP4)]),max([nanmean(theDataLRN2), nanmean(theDataSMP2)])],'k:');
title('Lag (ms) over trials');
xlabel('Trial');
ylabel('Lag (ms)');
legend('Learning Task (LERNEN)','Simple Task (UEBEN)');
...
theDataLRN5=cell2mat(SPFT.scoredData.errorlagcorrLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP5=cell2mat(SPFT.scoredData.errorlagcorrSMP); 
errorbar(nanmean(theDataLRN5),nanstd(theDataLRN5)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP5),nanstd(theDataSMP5)./sqrt(length(IDs)),'go-','linewidth',1);


plot([9.5 9.5], [min([nanmean(theDataLRN4), nanmean(theDataSMP4)]),max([nanmean(theDataLRN4), nanmean(theDataSMP2)])],'k:');
plot([18.5 18.5], [min([nanmean(theDataLRN5), nanmean(theDataSMP5)]),max([nanmean(theDataLRN5), nanmean(theDataSMP5)])],'k:');
plot([27.5 27.5], [min([nanmean(theDataLRN5), nanmean(theDataSMP5)]),max([nanmean(theDataLRN5), nanmean(theDataSMP5)])],'k:');
plot([36.5 36.5], [min([nanmean(theDataLRN5), nanmean(theDataSMP5)]),max([nanmean(theDataLRN5), nanmean(theDataSMP5)])],'k:');
plot([45.5 45.5], [min([nanmean(theDataLRN5), nanmean(theDataSMP5)]),max([nanmean(theDataLRN5), nanmean(theDataSMP5)])],'k:');
plot([54.5 54.5], [min([nanmean(theDataLRN5), nanmean(theDataSMP5)]),max([nanmean(theDataLRN5), nanmean(theDataSMP5)])],'k:');
xlabel('Trial');
ylabel('r');
legend('Learning Task (LERNEN)','Simple Task (UEBEN)');
...
theDataLRN=cell2mat(SPFT.scoredData.normrLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP=cell2mat(SPFT.scoredData.normrSMP); 
errorbar(nanmean(theDataLRN),nanstd(theDataLRN)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP),nanstd(theDataSMP)./sqrt(length(IDs)),'go-','linewidth',1);
...
theDataLRN6=cell2mat(SPFT.scoredData.lagLRN)./cell2mat(SPFT.scoredData.alignedrmseLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP6=cell2mat(SPFT.scoredData.lagSMP)./cell2mat(SPFT.scoredData.alignedrmseSMP); 
errorbar(nanmean(theDataLRN6),nanstd(theDataLRN6)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP6),nanstd(theDataSMP6)./sqrt(length(IDs)),'go-','linewidth',1);

plot([9.5 9.5], [min([nanmean(theDataLRN6), nanmean(theDataSMP6)]),max([nanmean(theDataLRN6), nanmean(theDataSMP6)])],'k:');
plot([18.5 18.5], [min([nanmean(theDataLRN6), nanmean(theDataSMP6)]),max([nanmean(theDataLRN6), nanmean(theDataSMP6)])],'k:');
plot([27.5 27.5], [min([nanmean(theDataLRN6), nanmean(theDataSMP6)]),max([nanmean(theDataLRN6), nanmean(theDataSMP6)])],'k:');
plot([36.5 36.5], [min([nanmean(theDataLRN6), nanmean(theDataSMP6)]),max([nanmean(theDataLRN6), nanmean(theDataSMP6)])],'k:');
plot([45.5 45.5], [min([nanmean(theDataLRN6), nanmean(theDataSMP6)]),max([nanmean(theDataLRN6), nanmean(theDataSMP6)])],'k:');
plot([54.5 54.5], [min([nanmean(theDataLRN6), nanmean(theDataSMP6)]),max([nanmean(theDataLRN6), nanmean(theDataSMP6)])],'k:');
xlabel('Trial');
ylabel('Lag/RMSE');
legend('Learning Task (LERNEN)','Simple Task (UEBEN)');
