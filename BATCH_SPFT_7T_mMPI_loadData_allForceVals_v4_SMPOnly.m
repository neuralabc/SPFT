...
addpath('/home/chris/Documents/projects_working/mMPI/scripts/bx');
%dataDir_root='/home/chris/Documents/projects_working/mMPI/source/bx/' %specify the root location for bx data, actual subdir will differ depending on the ID
dataDir_root='/home/chris/Downloads/XXXX/' %specify the root location for bx data, actual subdir will differ depending on the ID
...
SPFT.trialTypes=repmat([1 1 1 2 2 2 1 1 1],1,SPFT.numBlocks); %1=SMP, 2=RST, 3=LRN
...
SPFT.fileNameTail='-SPFT_7T_Daily_SMPonly.log';
clear temp;
...
subjectDirs={'XXXX'}; 
IDs={'P13'}; %Participants 13 and 15 are controls
...
for ID=1:length(IDs)
    ...
    for day=1:SPFT.numDays
        temp.fname=strcat(SPFT.dataDir,IDs{ID},'_d',num2str(day),SPFT.fileNameTail); %figure out what the trial name should be
        fprintf('%s\n',temp.fname);
        temp.day=day;
        if exist(temp.fname,'file')==2
            ...
            try
                ...
                if strcmp(IDs{ID},'P10') && day==4
                    temp.maxVal=322; %Sophia: corrected this value, if think it` s 322, not 305 (that` s the value for P11)
                    fprintf('%%========================================================%%\n');
...
for ID=1:length(IDs)
    ...
    for day=1:SPFT.numDays
        ...
        for trial=1:SPFT.numBlocks*SPFT.numTrials*2 %mult by 2 to take over the LRN trials, since we are only SMP
            fprintf('%i ',trial);
...
% only d1 to start
outDir='/home/chris/Documents/projects_working/mMPI/processing/bx/';
for ID=1:length(IDs)
    for day=1:1%6
        IDtxt=IDs{ID};
        cmd=['theVar=SPFT.',SPFT.IDs{ID},'.d1.out.rawForce_per_TR;'];
        eval(cmd);
        dlmwrite(strcat(outDir,IDtxt,'_d',num2str(day),'_rawForce_per_TR.txt'),theVar);
        
        cmd=['theVar=SPFT.',SPFT.IDs{ID},'.d1.GLM.LRNblocks;'];
        eval(cmd);
        theVar_LRNb=theVar;
        dlmwrite(strcat(outDir,IDtxt,'_d',num2str(day),'_LRNblocks.txt'),theVar);

        cmd=['theVar=SPFT.',SPFT.IDs{ID},'.d1.GLM.SMPblocks;'];
        eval(cmd);
        theVar_SMPb=theVar;
        dlmwrite(strcat(outDir,IDtxt,'_d',num2str(day),'_SMPblocks.txt'),theVar);
        
        %dlmwrite(strcat(outDir,IDtxt,'_d',num2str(day),'_LRN_SMPblocks_con.txt'),theVar);
...
SPFT_plotIndividualData_SMPonly(theDataSMP,plotName,yAxisName,SCALEX);
...
IDs=SPFT.IDs;
f=figure('name','RMSE');
theDataLRN3=cell2mat(SPFT.scoredData.rmseLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP3=cell2mat(SPFT.scoredData.rmseSMP); 
...
plot([72.5 72.5], [min([ nanmean(theDataSMP)]),max([ nanmean(theDataSMP)])],'k:');
plot([90.5 90.5], [min([ nanmean(theDataSMP)]),max([ nanmean(theDataSMP)])],'k:');
plot([108.5 108.5], [min([ nanmean(theDataSMP)]),max([ nanmean(theDataSMP)])],'k:');
...
theDataLRN=cell2mat(SPFT.scoredData.alignedrmseLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP=cell2mat(SPFT.scoredData.alignedrmseSMP); 
% errorbar(nanstd(theDataLRN)./sqrt(length(IDs)),'bo-','linewidth',1);
...
theDataLRN2=cell2mat(SPFT.scoredData.lagLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP2=cell2mat(SPFT.scoredData.lagSMP); 
...
plot([72.5 72.5], [min([nanmean(theDataLRN2), nanmean(theDataSMP2)]),max([nanmean(theDataLRN2), nanmean(theDataSMP2)])],'k:');
plot([90.5 90.5], [min([nanmean(theDataLRN2), nanmean(theDataSMP2)]),max([nanmean(theDataLRN2), nanmean(theDataSMP2)])],'k:');
plot([108.5 108.5], [min([nanmean(theDataLRN2), nanmean(theDataSMP2)]),max([nanmean(theDataLRN2), nanmean(theDataSMP2)])],'k:');
...
theDataLRN=cell2mat(SPFT.scoredData.errorlagcorrLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP=cell2mat(SPFT.scoredData.errorlagcorrSMP); 
% errorbar(nanstd(theDataLRN)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP),nanstd(theDataSMP)./sqrt(length(IDs)),'go-','linewidth',1);
...
theDataLRN=cell2mat(SPFT.scoredData.lagLRN)./cell2mat(SPFT.scoredData.alignedrmseLRN); %row=ID, column=trials across all days (i.e., 54)
theDataSMP=cell2mat(SPFT.scoredData.lagSMP)./cell2mat(SPFT.scoredData.alignedrmseSMP); 
% errorbar(nanstd(theDataLRN)./sqrt(length(IDs)),'bo-','linewidth',1);
hold on;
errorbar(nanmean(theDataSMP),nanstd(theDataSMP)./sqrt(length(IDs)),'go-','linewidth',1);
